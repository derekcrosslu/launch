...
function minilang(str) {
  const STACK = [];
  let REGISTER = 0;
  str.split(" ").reduce((acc, curr, index, arr) => {
    // n : Place a value, n, in the register. Do not modify the stack.
    if (!!Number(acc)) REGISTER = Number(acc);
    // PUSH : Push the register value onto the stack. Leave the value in the register.
    if (curr === "PUSH") STACK.push(REGISTER);
    // ADD : Pop a value from the stack and add it to the register value, storing the result in the register.
    if (curr === "ADD") REGISTER = REGISTER + STACK.pop();
    // SUB : Pop a value from the stack and subtract it from the register value, storing the result in the register.
    if (curr === "SUB") REGISTER = REGISTER - STACK.pop();
    // MULT : Pop a value from the stack and multiply it by the register value, storing the result in the register.
    if (curr === "MULT") REGISTER = REGISTER * STACK.pop();
    // DIV : Pop a value from the stack and divide the register value by the popped stack value, storing the integer result back in the register.
    if (curr === "DIV") REGISTER = Math.floor(REGISTER / STACK.pop());
    // REMAINDER : Pop a value from the stack and divide the register value by the popped stack value, storing the integer remainder of the division back in the register.
    if (curr === "REMAINDER") REGISTER = REGISTER % STACK.pop();
    // POP : Remove the topmost item from the stack and place it in the register.
    if (curr === "POP") REGISTER = STACK.pop();
    // PRINT : Print the register value.
    if (curr === "PRINT") console.log(REGISTER);
    return curr;
  }, 0);
  console.log(" ");
}
